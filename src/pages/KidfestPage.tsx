...
type CompDetail = {
  name: string; emoji: string; eligibility: string;
  rules: string[]; rewards: string; fee: string; notes?: string;
};

const competitionDetails: CompDetail[] = [
  { name: "Crawling Competition", emoji: "👶", eligibility: "Babies below 1 year who can crawl",
    rules: ["Safe crawling area provided", "Parents may cheer from sidelines", "Crawl to the finish line — fastest wins"],
    rewards: "Participation certificates · Trophies for top 3", fee: "₹100", notes: "Prior registration required · Limited slots available" },
  { name: "Tricycle Race", emoji: "🚲", eligibility: "Kids aged 2–5 years",
    rules: ["Ride a tricycle along the marked track", "No pushing or assistance allowed", "Fastest to finish wins"],
    rewards: "Medals for top 3 · Participation certificates for all", fee: "₹100" },
  { name: "Fancy Dress", emoji: "👗", eligibility: "Kids up to 10th standard",
    ...
    rewards: "Trophies for top 3 · Certificates for all participants", fee: "₹150" },
  { name: "Drawing Competition", emoji: "🎨", eligibility: "Kids up to 10th standard",
    rules: ["Theme announced on the day", "Materials provided", "Time limit: 45 minutes"],
    rewards: "Prizes for top 3 in each age group · Certificates for all", fee: "₹100" },
  { name: "Little Prince & Princess Contest", emoji: "👑", eligibility: "Kids aged 3–10 years",
    rules: ["Judged on personality, confidence & stage presence", "Short Q&A round included", "Traditional or western attire welcome"],
    rewards: "Crown, sash & trophy for winners · Certificates for all", fee: "₹200" },
  { name: "Robotics Contest", emoji: "🤖", eligibility: "Kids aged 8–16 years",
    rules: ["Build or program a robot for a given task", "Teams of 1–3 allowed", "Materials may be brought or provided on-site"],
    rewards: "Trophies & certificates · Special innovation award", fee: "₹250" },
  { name: "Quiz Contest", emoji: "🧠", eligibility: "Kids aged 6–16 years",
    rules: ["General knowledge + current affairs", "Individual participation", "Multiple rounds — elimination format"],
    rewards: "Trophies for top 3 · Certificates for all", fee: "₹100" },
  { name: "Twin Show", emoji: "👯", eligibility: "Twins & Triplets of any age",
    ...
    rewards: "Special Twin Trophy · Certificates for all pairs", fee: "₹150 per pair", notes: "Triplets also welcome — special category available" },
  { name: "Marathon", emoji: "🏃", eligibility: "Kids aged 5–16 years (age-group categories)",
    rules: ["Marked route inside the venue", "Age-group heats", "Safety marshals present throughout"],
    rewards: "Medals for top 3 in each age group · Finisher certificates", fee: "₹100" },
];
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <motion.div className="relative z-10 w-full max-w-lg bg-card rounded-3xl shadow-elegant overflow-hidden max-h-[90vh] overflow-y-auto"
          initial={{ scale: 0.88, opacity: 0, y: 24 }} animate={{ scale: 1, opacity: 1, y: 0 }}
          exit={{ scale: 0.88, opacity: 0, y: 24 }} transition={{ type: "spring", stiffness: 320, damping: 28 }}
          onClick={(e) => e.stopPropagation()}>
          <div className="bg-gradient-primary p-6 text-primary-foreground relative">
            <button onClick={onClose} className="absolute top-4 right-4 h-8 w-8 rounded-full bg-white/20 hover:bg-white/30 grid place-items-center transition-colors" aria-label="Close">
              <X className="h-4 w-4" />
            </button>
          ...
          <div className="p-6 space-y-5">
              ...
              <ul className="space-y-1.5">
                {detail.rules.map((r, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
                    <span className="mt-1 h-1.5 w-1.5 rounded-full bg-primary flex-shrink-0" />{r}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="flex gap-3">
              ...
              {detail.notes && (
                <div className="flex-1 p-4 rounded-2xl bg-secondary border border-border text-sm text-muted-foreground flex items-center">{detail.notes}</div>
              )}
            </div>
          </div>
        </motion.div>
...
function DetailRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div>
      <p className="text-xs uppercase tracking-wider text-primary font-semibold mb-1 flex items-center gap-1.5">{icon} {label}</p>
...
function HappeningCard({ h }: { h: typeof happenings[number] }) {
  ...
  return (
    <div className="group relative rounded-2xl overflow-hidden bg-card border border-border h-44 cursor-default"
      style={{ transition: "transform 0.35s cubic-bezier(.2,.8,.2,1), box-shadow 0.35s" }}
      onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      {h.img && <img src={h.img} alt="" className="absolute inset-0 h-full w-full object-cover opacity-40 group-hover:opacity-60 transition-opacity duration-500" loading="lazy" />}
...
export default function KidfestPage() {
      ...
      <section className="py-20 px-6 lg:px-10 bg-background">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {happenings.map((h, i) => (
              <Reveal key={h.title} delay={i * 60}><HappeningCard h={h} /></Reveal>
            ))}
          </div>
      ...
      <section className="py-20 px-6 lg:px-10 bg-background">
        <div className="mx-auto max-w-7xl grid lg:grid-cols-2 gap-12 items-center">
          ...
          <Reveal delay={150}>
            ...
            <div className="mt-8">
              <Button size="lg" onClick={() => openWith("Crawling Competition")}
          ...
          <Reveal className="mb-10">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-5 rounded-2xl bg-card border border-border shadow-soft">
              ...
              <motion.div animate={{ scale: [1, 1.04, 1] }} transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}>
                <Button onClick={() => openWith()} size="lg"
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {competitions.map((c, i) => (
              <Reveal key={c.name} delay={i * 50}>
                <motion.div className="group p-6 rounded-2xl bg-card border border-border flex items-center gap-4 cursor-pointer"
                  ...
                  <motion.button onClick={(e) => { e.stopPropagation(); openWith(c.name as any); }}
                    ...
                    <motion.span className="absolute inset-0 rounded-full bg-white/20"
                      initial={{ scale: 0, opacity: 0.6 }} whileTap={{ scale: 2.5, opacity: 0 }} transition={{ duration: 0.4 }} />
                  </motion.button>
                </motion.div>
              </Reveal>
            ))}
